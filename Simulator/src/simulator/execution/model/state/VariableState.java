...
public class VariableState implements Serializable {
	...
	private final Map<VariableKey, Time> variableValues = new HashMap<VariableKey, Time>();

	public Time getValueOf(VariableKey key) {
	...
	public Collection<VariableWithValue> getValues(int currentModeIndex) {
		...
		for (Entry<VariableKey, Time> variableValue : variableValues.entrySet()) {
	...
	public void setValueOf(VariableKey key, Time value) {
		variableValues.put(key, value);
	}
	
	public void initialiseValueOf(VariableKey key) {
		if (!variableValues.containsKey(key)) variableValues.put(key, new Time());
	}
