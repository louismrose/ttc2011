 ...
 */
public class SslOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "SimulatorSpecificationLanguage.resource.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "SimulatorSpecificationLanguage.resource.ui.occurences.declaration";
	
	private final static ssl.resource.ssl.ui.SslPositionHelper positionHelper = new ssl.resource.ssl.ui.SslPositionHelper();
	
	private ssl.resource.ssl.ui.SslTokenScanner tokenScanner;
	...
	private ssl.resource.ssl.ISslTextResource textResource;
	 ...
	 */
	public SslOccurrence(ssl.resource.ssl.ISslTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, ssl.resource.ssl.ui.SslTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new ssl.resource.ssl.mopp.SslMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(ssl.resource.ssl.ui.SslBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, ssl.resource.ssl.ui.SslPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, ssl.resource.ssl.ui.SslPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, ssl.resource.ssl.ui.SslPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == ssl.resource.ssl.ui.SslPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(ssl.resource.ssl.ui.SslOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(ssl.resource.ssl.ui.SslOccurrence.DECLARATION_ANNOTATION_ID);
	}
