...
public class SslBracketSet {
	...
	private java.util.ArrayList<ssl.resource.ssl.ISslBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements ssl.resource.ssl.ISslBracketPair {
	 ...
	 */
	private class ClosingListener implements ssl.resource.ssl.ui.ISslBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				ssl.resource.ssl.ISslBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public SslBracketSet(ssl.resource.ssl.ui.SslEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new ssl.resource.ssl.mopp.SslMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<ssl.resource.ssl.ISslBracketPair>();
	 ...
	 */
	public ssl.resource.ssl.ISslBracketPair getBracketPair(String opening, String closing) {
		for (ssl.resource.ssl.ISslBracketPair bracketPair : bracketPairs) {
	...
	public ssl.resource.ssl.ISslBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(ssl.resource.ssl.ISslBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + ssl.resource.ssl.ui.SslPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public ssl.resource.ssl.ISslBracketPair remove(String opening, String closing) {
		for (ssl.resource.ssl.ISslBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<ssl.resource.ssl.ISslBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, ssl.resource.ssl.ui.SslPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, ssl.resource.ssl.ui.SslPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
