...
/**
 * A text editor for 'ssl' models.
 */
public class SslEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider {
	
	private ssl.resource.ssl.ui.SslHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private ssl.resource.ssl.ui.SslCodeFoldingManager codeFoldingManager;
	private ssl.resource.ssl.ui.SslBackgroundParsingStrategy bgParsingStrategy = new ssl.resource.ssl.ui.SslBackgroundParsingStrategy();
	private java.util.Collection<ssl.resource.ssl.ISslBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<ssl.resource.ssl.ISslBackgroundParsingListener>();
	private ssl.resource.ssl.ui.SslColorManager colorManager = new ssl.resource.ssl.ui.SslColorManager();
	private ssl.resource.ssl.ui.SslOutlinePage outlinePage;
	...
	private ssl.resource.ssl.ui.SslPropertySheetPage propertySheetPage;
	...
	private ssl.resource.ssl.ui.ISslBracketHandler bracketHandler;
	
	public SslEditor() {
		super();
		setSourceViewerConfiguration(new ssl.resource.ssl.ui.SslEditorConfiguration(this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), SslEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								ssl.resource.ssl.ISslTextResource currentResource = getResource();
				...
				ssl.resource.ssl.ui.SslUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("SimulatorSpecificationLanguage.resource.EditorContext");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new ssl.resource.ssl.ui.SslHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new ssl.resource.ssl.ui.SslCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		ssl.resource.ssl.mopp.SslNature.activate(inputFile.getProject());
		...
		ssl.resource.ssl.ISslTextResource loadedResource = (ssl.resource.ssl.ISslTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				ssl.resource.ssl.ISslTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof ssl.resource.ssl.ISslTextResource) {
					setResource((ssl.resource.ssl.ISslTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					ssl.resource.ssl.ui.SslUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the SslEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				ssl.resource.ssl.ui.SslUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(ssl.resource.ssl.ISslTextResource resource) {
		assert resource != null;
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new ssl.resource.ssl.ui.SslOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new ssl.resource.ssl.ui.SslPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			ssl.resource.ssl.ISslTextResource textResource = (ssl.resource.ssl.ISslTextResource) element.eResource();
			...
			ssl.resource.ssl.ISslTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				ssl.resource.ssl.ISslTextToken token = lexer.getNextToken();
			...
			ssl.resource.ssl.ui.SslUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	public void addBackgroundParsingListener(ssl.resource.ssl.ISslBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (ssl.resource.ssl.ISslBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public ssl.resource.ssl.ui.ISslBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(ssl.resource.ssl.ui.ISslBracketHandler bracketHandler) {
