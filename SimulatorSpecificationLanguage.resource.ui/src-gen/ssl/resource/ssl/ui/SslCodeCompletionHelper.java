 ...
 */
public class SslCodeCompletionHelper {
	
	private ssl.resource.ssl.mopp.SslAttributeValueProvider attributeValueProvider = new ssl.resource.ssl.mopp.SslAttributeValueProvider();
	 ...
	 * @return
	 */
	public ssl.resource.ssl.ui.SslCompletionProposal[] computeCompletionProposals(ssl.resource.ssl.ISslTextResource originalResource, String content, int cursorOffset) {
		...
		ssl.resource.ssl.ISslTextResource resource = (ssl.resource.ssl.ISslTextResource) resourceSet.createResource(originalResource.getURI());
		...
		ssl.resource.ssl.ISslTextParser parser = metaInformation.createParser(inputStream, null);
		ssl.resource.ssl.mopp.SslExpectedTerminal[] expectedElements = parseToExpectedElements(parser, resource, cursorOffset);
		if (expectedElements == null) {
			return new ssl.resource.ssl.ui.SslCompletionProposal[0];
		}
		if (expectedElements.length == 0) {
			return new ssl.resource.ssl.ui.SslCompletionProposal[0];
		}
		java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedAfterCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset));
		java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedBeforeCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset - 1));
		...
		java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> allProposals = new java.util.LinkedHashSet<ssl.resource.ssl.ui.SslCompletionProposal>();
		java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> rightProposals = deriveProposals(expectedAfterCursor, content, resource, cursorOffset);
		java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> leftProposals = deriveProposals(expectedBeforeCursor, content, resource, cursorOffset - 1);
		...
		final java.util.List<ssl.resource.ssl.ui.SslCompletionProposal> sortedProposals = new java.util.ArrayList<ssl.resource.ssl.ui.SslCompletionProposal>(allProposals);
		java.util.Collections.sort(sortedProposals);
		return sortedProposals.toArray(new ssl.resource.ssl.ui.SslCompletionProposal[sortedProposals.size()]);
	}
	
	public ssl.resource.ssl.mopp.SslExpectedTerminal[] parseToExpectedElements(ssl.resource.ssl.ISslTextParser parser, ssl.resource.ssl.ISslTextResource resource, int cursorOffset) {
		final java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedElements = parser.parseToExpectedElements(null, resource, cursorOffset);
		if (expectedElements == null) {
			return new ssl.resource.ssl.mopp.SslExpectedTerminal[0];
		}
		removeDuplicateEntries(expectedElements);
		removeInvalidEntriesAtEnd(expectedElements);
		return expectedElements.toArray(new ssl.resource.ssl.mopp.SslExpectedTerminal[expectedElements.size()]);
	}
	
	private void removeDuplicateEntries(java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1; i++) {
			ssl.resource.ssl.mopp.SslExpectedTerminal elementAtIndex = expectedElements.get(i);
			for (int j = i + 1; j < expectedElements.size();) {
				ssl.resource.ssl.mopp.SslExpectedTerminal elementAtNext = expectedElements.get(j);
	...
	private void removeInvalidEntriesAtEnd(java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1;) {
			ssl.resource.ssl.mopp.SslExpectedTerminal elementAtIndex = expectedElements.get(i);
			ssl.resource.ssl.mopp.SslExpectedTerminal elementAtNext = expectedElements.get(i + 1);
	...
	private String findPrefix(java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedElements, ssl.resource.ssl.mopp.SslExpectedTerminal expectedAtCursor, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return "";
		}
		int end = 0;
		for (ssl.resource.ssl.mopp.SslExpectedTerminal expectedElement : expectedElements) {
	...
	private java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> deriveProposals(java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedElements, String content, ssl.resource.ssl.ISslTextResource resource, int cursorOffset) {
		java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> resultSet = new java.util.LinkedHashSet<ssl.resource.ssl.ui.SslCompletionProposal>();
		for (ssl.resource.ssl.mopp.SslExpectedTerminal expectedElement : expectedElements) {
	...
	private java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> handleEnumAttribute(ssl.resource.ssl.ISslMetaInformation metaInformation, ssl.resource.ssl.mopp.SslExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EEnum enumType, String prefix, org.eclipse.emf.ecore.EObject container) {
		java.util.Collection<org.eclipse.emf.ecore.EEnumLiteral> enumLiterals = enumType.getELiterals();
		java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> result = new java.util.LinkedHashSet<ssl.resource.ssl.ui.SslCompletionProposal>();
		for (org.eclipse.emf.ecore.EEnumLiteral literal : enumLiterals) {
			...
			ssl.resource.ssl.ISslTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
			ssl.resource.ssl.ISslTokenResolver tokenResolver = tokenResolverFactory.createTokenResolver(expectedFeature.getTokenName());
			...
			result.add(new ssl.resource.ssl.ui.SslCompletionProposal(resolvedLiteral, prefix, matchesPrefix, expectedFeature.getFeature(), container));
		}
		return result;
	}
	
	private java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> handleNCReference(ssl.resource.ssl.ISslMetaInformation metaInformation, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, String prefix, String tokenName) {
		// proposals for non-containment references are derived by calling the reference
		// resolver switch in fuzzy mode.
		ssl.resource.ssl.ISslReferenceResolverSwitch resolverSwitch = metaInformation.getReferenceResolverSwitch();
		ssl.resource.ssl.ISslTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
		ssl.resource.ssl.ISslReferenceResolveResult<org.eclipse.emf.ecore.EObject> result = new ssl.resource.ssl.mopp.SslReferenceResolveResult<org.eclipse.emf.ecore.EObject>(true);
		resolverSwitch.resolveFuzzy(prefix, container, reference, 0, result);
		java.util.Collection<ssl.resource.ssl.ISslReferenceMapping<org.eclipse.emf.ecore.EObject>> mappings = result.getMappings();
		if (mappings != null) {
			java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> resultSet = new java.util.LinkedHashSet<ssl.resource.ssl.ui.SslCompletionProposal>();
			for (ssl.resource.ssl.ISslReferenceMapping<org.eclipse.emf.ecore.EObject> mapping : mappings) {
				org.eclipse.swt.graphics.Image image = null;
				if (mapping instanceof ssl.resource.ssl.mopp.SslElementMapping<?>) {
					ssl.resource.ssl.mopp.SslElementMapping<?> elementMapping = (ssl.resource.ssl.mopp.SslElementMapping<?>) mapping;
					...
					resultSet.add(new ssl.resource.ssl.ui.SslCompletionProposal(identifier, prefix, matchesPrefix, reference, container, image));
				}
			}
			return resultSet;
		}
				...
				if (defaultValue != null) {
					...
					if (tokenName != null) {
						...
						if (tokenResolver != null) {
							...
							resultSet.add(new ssl.resource.ssl.ui.SslCompletionProposal(defaultValueAsString, prefix, matchesPrefix, expectedFeature.getFeature(), container));
						}
					}
				}
			}
	...
	private java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> handleKeyword(ssl.resource.ssl.mopp.SslExpectedCsString csString, String prefix) {
		String proposal = csString.getValue();
		boolean matchesPrefix = matches(proposal, prefix);
		return java.util.Collections.singleton(new ssl.resource.ssl.ui.SslCompletionProposal(proposal, prefix, matchesPrefix, null, null));
	}
	
	private java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> handleBooleanTerminal(ssl.resource.ssl.mopp.SslExpectedBooleanTerminal expectedBooleanTerminal, String prefix) {
		java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> result = new java.util.LinkedHashSet<ssl.resource.ssl.ui.SslCompletionProposal>(2);
		ssl.resource.ssl.grammar.SslBooleanTerminal booleanTerminal = expectedBooleanTerminal.getBooleanTerminal();
	...
	private java.util.Collection<ssl.resource.ssl.ui.SslCompletionProposal> handleBooleanLiteral(org.eclipse.emf.ecore.EAttribute attribute, String prefix, String literal) {
		if ("".equals(literal)) {
			return java.util.Collections.emptySet();
		}
		boolean matchesPrefix = matches(literal, prefix);
		return java.util.Collections.singleton(new ssl.resource.ssl.ui.SslCompletionProposal(literal, prefix, matchesPrefix, null, null));
	}
	 ...
	 * element is expected.
	 */
	private void setPrefixes(java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedElements, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return;
		}
		for (ssl.resource.ssl.mopp.SslExpectedTerminal expectedElement : expectedElements) {
	...
	public ssl.resource.ssl.mopp.SslExpectedTerminal[] getElementsExpectedAt(ssl.resource.ssl.mopp.SslExpectedTerminal[] allExpectedElements, int cursorOffset) {
		java.util.List<ssl.resource.ssl.mopp.SslExpectedTerminal> expectedAtCursor = new java.util.ArrayList<ssl.resource.ssl.mopp.SslExpectedTerminal>();
		for (int i = 0; i < allExpectedElements.length; i++) {
			ssl.resource.ssl.mopp.SslExpectedTerminal expectedElement = allExpectedElements[i];
		...
		return expectedAtCursor.toArray(new ssl.resource.ssl.mopp.SslExpectedTerminal[expectedAtCursor.size()]);
	}
	
	private int getEnd(ssl.resource.ssl.mopp.SslExpectedTerminal[] allExpectedElements, int indexInList) {
		ssl.resource.ssl.mopp.SslExpectedTerminal elementAtIndex = allExpectedElements[indexInList];
		...
		for (int i = indexInList + 1; i < allExpectedElements.length; i++) {
			ssl.resource.ssl.mopp.SslExpectedTerminal elementAtI = allExpectedElements[i];
	...
	private boolean matches(String proposal, String prefix) {
		return (proposal.toLowerCase().startsWith(prefix.toLowerCase()) || ssl.resource.ssl.util.SslStringUtil.matchCamelCase(prefix, proposal) != null) && !proposal.equals(prefix);
	}
