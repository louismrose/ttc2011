 ...
 */
public class SslHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static ssl.resource.ssl.ui.SslPositionHelper positionHelper = new ssl.resource.ssl.ui.SslPositionHelper();
	private boolean isHighlightBrackets = true;
	private ssl.resource.ssl.ui.SslTokenScanner scanner;
	...
	private ssl.resource.ssl.ui.SslOccurrence occurrence;
	private ssl.resource.ssl.ui.SslBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, ssl.resource.ssl.ISslBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public SslHighlighting(ssl.resource.ssl.ISslTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, ssl.resource.ssl.ui.SslColorManager colorManager, ssl.resource.ssl.ui.SslEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = ssl.resource.ssl.ui.SslUIPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new ssl.resource.ssl.ui.SslTokenScanner(textResource, colorManager);
		occurrence = new ssl.resource.ssl.ui.SslOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new ssl.resource.ssl.ui.SslBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(ssl.resource.ssl.ui.SslPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, ssl.resource.ssl.ui.SslPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(ssl.resource.ssl.ui.SslEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, ssl.resource.ssl.ui.SslPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, ssl.resource.ssl.ui.SslPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(ssl.resource.ssl.ui.SslPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new ssl.resource.ssl.ui.SslEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof ssl.resource.ssl.ISslTextResource) {
